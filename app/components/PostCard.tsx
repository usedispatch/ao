...
export const PostCard = ({ post, depth = 0, createPost, handleLike, handleUnlike, profile }: PostCardProps) => {
  ...
  return (
    <Card className={`overflow-hidden bg-[#FAFAF8] shadow-sm hover:shadow-md transition-shadow duration-200 ${depth > 0 ? 'ml-6' : ''}`}>
      ...
      <CardFooter className="flex justify-between bg-[#F1F0EA] p-2">
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button
            variant="ghost"
            size="sm"
            onClick={() => (post.Likes ? handleUnlike(post.Id) : handleLike(post.Id))}
            className={`text-[#141414] hover:bg-[#FAFAF8] transition-colors duration-200 ${
              post.Likes && post.Likes > 0 ? "text-[#CE775A]" : ""
            }`}
          >
            {post.Likes && post.Likes > 0 ? (
              <Heart className="w-4 h-4 mr-2" />
            ) : (
              <Heart className="w-4 h-4 mr-2 text-[#141414]" />
            )}
            {post.Likes}
          </Button>
        </motion.div>
      ...
      {post.Replies && post.Replies.map((reply: Post) => (
        <PostCard
          key={reply.Id}
          post={reply}
          depth={depth + 1}
